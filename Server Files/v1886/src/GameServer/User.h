...
enum ClassType
{
	...
	ClassKurian         = 6,
	ClassWarriorMaster	= 7,
	ClassRogueNovice	= 8,
	ClassRogueMaster	= 9,
	ClassMageNovice		= 10,
	ClassMageMaster		= 11,
	ClassPriestNovice	= 12,
	ClassPriestMaster	= 13,
	ClassPortu          = 14,
	ClassKurianNovice   = 15,
	ClassKurianMaster   = 16,
	ClassPortuNovice    = 17,
	ClassPortuMaster    = 18
};
...
public:
	...
	INLINE bool isKurian() { return JobGroupCheck(ClassKurian); }
	INLINE bool isPortu() { return JobGroupCheck(ClassPortu); }
	...
	INLINE bool isBeginnerKurian()  { return GetClassType() == ClassKurian; }
	INLINE bool isBeginnerPortu()  { return GetClassType() == ClassPortu; }
	...
	INLINE bool isNovice() 
	{
		...
		return (sClass == ClassWarriorNovice || sClass == ClassRogueNovice
			|| sClass == ClassMageNovice || sClass == ClassPriestNovice || sClass == ClassKurianNovice || sClass == ClassPortuNovice); 
	}
#else
	INLINE bool isNovice() 
	{
	...
	INLINE bool isNoviceKurian()  { return GetClassType() == ClassKurianNovice; }
	INLINE bool isNovicePortu()  { return GetClassType() == ClassPortuNovice; }
	...
	INLINE bool isMastered() 
	{
		...
		return (sClass == ClassWarriorMaster || sClass == ClassRogueMaster 
			|| sClass == ClassMageMaster || sClass == ClassPriestMaster || sClass == ClassKurianMaster || sClass == ClassPortuMaster );
	}
#else
	INLINE bool isMastered() 
	{
	...
	INLINE bool isMasteredKurian()  { return GetClassType() == ClassKurianMaster; }
	INLINE bool isMasteredPortu()  { return GetClassType() == ClassPortuMaster; }
	...
	*/
	INLINE ClassType GetBaseClassType()
	{
		static const ClassType classTypes[] = 
		{
			...
			ClassKurian, ClassKurian,  // job changed / mastered
			ClassPortu, ClassPortu      // job changed / mastered
		}; 
	...
	*/
	...
	bool GiveItem(uint32 nItemID, uint16 sCount = 1, bool send_packet = true, int isRentalHour = 0);
	bool GiveRentalItem(uint32 nItemID, uint16 sCount = 1, bool send_packet = true);
	...
	void SendStackChange(uint32 nItemID, uint32 nCount /* needs to be 4 bytes, not a bug */, uint16 sDurability, uint8 bPos, bool bNewItem = false,uint32 ExpirationTim=0);
    void kontrol_sureli_item();
	void Type4Duration();
	void HPTimeChange();
