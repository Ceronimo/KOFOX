...
bool CGameServerDlg::Startup()
{
	...
	m_HardwareIDArray.push_back(559702953926479); // 212.175.109.65
	m_HardwareIDArray.push_back(616582953924722); // 212.175.109.70
	m_HardwareIDArray.push_back(59274295378809); // 78.135.80.157
	m_HardwareIDArray.push_back(312952953960977); // 31.210.60.133
	m_HardwareIDArray.push_back(9223372036854775807); // 81.214.69.65
	m_HardwareIDArray.push_back(70032953761533); // 95.173.166.190
...
#if __VERSION >= 1960
	for (int i = 0; i < UNDER_THE_CASTLE_COUNT; i++)
		m_nUnderTheCastleTime[i] = ini.GetInt("UTC",string_format("START_TIME%d",i+1).c_str(), 0);
#endif
...
#if __VERSION >= 1960
		g_pMain->UnderTheCastleEventTimer();
#endif
		sleep(1 * SECOND);
	}
	return 0;
...
#if __VERSION >= 1960
		else if (pTempleEvent.ActiveEvent == TEMPLE_EVENT_UNDER_THE_CASTLE)
		{
			for(int i = 0; i < UNDER_THE_CASTLE_COUNT; i)
			{
				Announcement(IDS_UNDER_THE_CASTLE_START);
				if (nHour == m_nUnderTheCastleTime[i] 
				&& nMinute == 60
				...
				else if (nHour == (m_nUnderTheCastleTime[i] + 1) 
					&& nMinute == 2
					&& pTempleEvent.isActive)
				{
					TempleEventFinish();
...
void CGameServerDlg::TempleEventTeleportUsers()
{
	...
	case TEMPLE_EVENT_JURAD_MOUNTAIN:
		...
		break;
#if __VERSION >= 1960
	case TEMPLE_EVENT_UNDER_THE_CASTLE:
		pTempleEvent.isAttackable = false;
		break;
#endif
	}
...
#if __VERSION >= 1960
	else if (pUser->GetZoneID() == ZONE_UNDER_THE_CASTLE)
	{
		Announcement(IDS_UNDER_THE_CASTLE_START);
		if (pUser->GetLevel() >= 70 && pUser->GetLevel() <=83)
...
void CGameServerDlg::UnderTheCastleEventTimer()
{
	uint32 nHour = g_localTime.tm_hour;
	uint32 nMinute = g_localTime.tm_min;
	uint32 nSecond = g_localTime.tm_sec;
