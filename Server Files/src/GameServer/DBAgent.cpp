...
void CDBAgent::LoadCharInfo(string & strCharID, ByteBuffer & result)
{
	...
	if (!strCharID.empty())
	{
		...
		if (!dbCommand->Execute(_T("SELECT Race, Class, HairRGB, [Level], Face, Zone, strItem, strItemEx FROM USERDATA WHERE strUserID = ?")))
			ReportSQLError(m_GameDB->GetError());
...
bool CDBAgent::LoadUserData(string & strAccountID, string & strCharID, CUser *pUser)
{
	...
	char strItem[INVENTORY_TOTAL * 8] = {0}, strItemEx[INVENTORY_TOTAL * 8] = {0}, strSerial[INVENTORY_TOTAL * 8] = {0},
...
bool CDBAgent::LoadWarehouseData(string & strAccountID, CUser *pUser)
{
	...
	if (!dbCommand->Execute(_T("SELECT nMoney, WarehouseData,  WarehouseDataEx, strSerial FROM WAREHOUSE WHERE strAccountID = ?")))
		ReportSQLError(m_GameDB->GetError());
...
bool CDBAgent::UpdateUser(string & strCharID, UserUpdateType type, CUser *pUser)
{
	...
	ByteBuffer itemBuffer, serialBuffer, itemEx;
	for (int i = 0; i < INVENTORY_TOTAL; i++)
	{
		_ITEM_DATA *pItem = &pUser->m_sItemArray[i];
		itemBuffer << pItem->nNum << pItem->sDuration << pItem->sCount;
		itemEx<< pItem->nExpirationTime << pItem->sRemainingRentalTime;
	...
	dbCommand->AddParameter(SQL_PARAM_INPUT, (char *)itemEx.contents(), itemEx.size(), SQL_BINARY);
	...
	if (!dbCommand->Execute(string_format(_T("{CALL UPDATE_USER_DATA("
		...
		"?, ?, ?, ?, ?, "			// strSkill, strItem, strExpiration, strSerial, strQuest
...
bool CDBAgent::UpdateWarehouseData(string & strAccountID, UserUpdateType type, CUser *pUser)
{
	...
	if (!dbCommand->Execute(string_format(_T("UPDATE WAREHOUSE SET nMoney=%d, dwTime=%d, WarehouseData=?, WarehouseDataEx=?, strSerial=? WHERE strAccountID=?"),
