...
void CUser::MonsterStoneProcess()
{
	if(CheckExistItem(ITEM_MONSTER_STONE,1))
	{
		...
		if (GetLevel() >= 20 && GetLevel() <=29)
			ZoneChange(myrand(ZONE_KECOONS_BOSS,ZONE_DEGENERATES_NEST),0.0f,0.0f);
        if (GetLevel() >= 30 && GetLevel() <=39)
			ZoneChange(myrand(ZONE_SOMINARK_FOREST,ZONE_SOGODDES_IMIR),0.0f,0.0f);
        if (GetLevel() >= 40 && GetLevel() <=49)
			ZoneChange(myrand(ZONE_BIRTH_OF_DRAGON,ZONE_MOANCIENT_PALADIN),0.0f,0.0f);
        if (GetLevel() >= 50 && GetLevel() <=59)
			ZoneChange(myrand(ZONE_SOPIANA_KNIGHTS,ZONE_HARPYS_NEST),0.0f,0.0f);
        if (GetLevel() >= 58 && GetLevel() <=60)
			ZoneChange(myrand(ZONE_EKIONTRMAGICIAN,ZONE_MOATREASURE_HUNTER),0.0f,0.0f);
        if (GetLevel() >= 61 && GetLevel() <=72)
			ZoneChange(myrand(ZONE_CURSED_CRYSTAL,ZONE_KNIGHTSINBLOOD_RED),0.0f,0.0f);
        if (GetLevel() >= 73 && GetLevel() <=83)
			ZoneChange(ZONE_FSUBJUGATION_FORCE,0.0f,0.0f);
	}
} 
